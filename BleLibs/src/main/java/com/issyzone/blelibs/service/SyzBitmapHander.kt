...
import com.issyzone.blelibs.dataimp.BlePrintingCallBack
import com.issyzone.blelibs.utils.BitmapUtils
import com.issyzone.blelibs.utils.HeatShrinkUtils
import com.issyzone.blelibs.utils.SyzBitmapQueue
import com.issyzone.blelibs.utils.SyzPrinterSetting
...
/**
 * 是位图处理器的主类，它使用了构建者模式来设置打印机的类型、位图的宽度、高度和打印页数。
 */
class SyzBitmapHander private constructor(var builder: Builder) {
    companion object {
    ...
    class Builder {
        ...
        fun build() = SyzBitmapHander(this)
    }
     ...
     * ABC  page=2  ABC ABC
     */
    suspend fun produceBitmaps(bitmapList: MutableList<Bitmap>): SyzBitmapHander {
        initPrinterParams()
     ...
     */
    suspend fun producePDFFile(bitmapList: MutableList<Bitmap>): SyzBitmapHander {
        initPrinterParams()
        val printTaskLength = bitmapList.size * (builder.printPage)
        bitMapTaskQueue = SyzBitmapQueue(printTaskLength)
        totalPage = printTaskLength
        Log.d(TAG, "准备PDF打印机任务>>${bitmapList.size}张bitmap,打印${builder.printPage}份==总共要打印${totalPage}份")
        bitmapList.forEachIndexed { index, bitmap ->
            val duanListQueue = turnBitmapToDuanList(bitmap, 1, index)
            if (bitmap != null && !(bitmap.isRecycled)) {
                bitmap.recycle()//回收
    ...
    private var bitmapCall: BlePrintingCallBack? = null //打印回调
    fun setBimapCallBack(callBack: BlePrintingCallBack): SyzBitmapHander {
    ...
    private suspend fun processBitmap(
        ...
        if (bitmapProcessed == 1) {
            ...
            val stateFlag =SyzPrinterState2.PRINTER_OK
    ...
    suspend fun consumeDuansOneBitmap() = withContext(Dispatchers.IO) {
        if (currentBitmapQueue.isEmpty()) {
           ...
           // SyzBleManager.getInstance().fmWriteABF4(duanData)
        }
    }

//    private suspend fun checkPrinterState(): SyzPrinterState2 {
//        return SyzBleManager.getInstance().getPrintStatus()
//    }
