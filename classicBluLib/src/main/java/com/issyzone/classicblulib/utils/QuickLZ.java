...
public final class QuickLZ {
    ...
    static public long sizeDecompressed(byte[] source) {
        if (headerLen(source) == 9) return fast_read(source, 5, 4);
        else return fast_read(source, 2, 1);
    }

    static public long sizeCompressed(byte[] source) {
        if (headerLen(source) == 9) return fast_read(source, 1, 4);
        else return fast_read(source, 1, 1);
    }
    ...
    public static byte[] compress(byte[] source, int level) {
        ...
        if (src <= last_matchstart) fetch = (int) fast_read(source, src, 3);
            ...
            if (level == 1) {
                ...
                for (k = 0; k < QLZ_POINTERS_3 && (c > k || c < 0); k++) {
                    ...
                    if ((byte) fetch == source[o] && (byte) (fetch >>> 8) == source[o + 1] && (byte) (fetch >>> 16) == source[o + 2] && o < src - MINOFFSET) {
                        m = 3;
                        while (source[o + m] == source[src + m] && m < remaining) m++;
        ...
        for (; ; ) {
            if (cword_val == 1) {
                ...
                if (dst <= last_matchstart) {
                    if (level == 1) fetch = (int) fast_read(source, src, 3);
                    else fetch = (int) fast_read(source, src, 4);
                }
            }
