...
class SyzBitmapProcessor private constructor(var builder: Builder) {
    ...
    suspend fun produceBitmaps2(bitmapList: MutableList<Bitmap>): SyzBitmapProcessor {
        ...
        bitmapList.forEachIndexed { index, bitmap ->
            if (!currentCoroutineContext().isActive) return this // 检查协程是否被取消// 检查协程是否被取消
...
//                    val filePath = File(
//                        AppGlobels.getApplication().externalCacheDir, "sdk_print"
//                    ).apply { mkdirs() }.absolutePath + File.separator + "print-" + System.currentTimeMillis() + ".png"
//
//                    bitmap2Path(cropBitmap, filePath)
            ...
            if (!isPrintMore) {
                //单图
                val duanREsult = turnBitmapToDuanList(newBitmap, builder.printPage, index,false)
                val duanListQueue = duanREsult.second
                isSupportPageMore = duanREsult.first
                bitMapTaskQueue.add(duanListQueue)
            } else {
                //多张图
                val duanREsult = turnBitmapToDuanList(newBitmap, 1, index,true)
                val duanListQueue = duanREsult.second
                isSupportPageMore = duanREsult.first
                bitMapTaskQueue.add(duanListQueue)
            }
        ...
        if (!isPrintMore) {
            ...
            if (isSupportPageMore) {
                Log.i(
                    TAG,
                    "打印机${builder.printerType}打印任务>>${bitMapTaskQueue.size()}张bitmap,一张${builder.printPage}份==总共要打印${totalPage}份==是否支持page多份${isSupportPageMore}"
                )
            } else {
                ...
                Log.i(
                    TAG,
                    "打印机${builder.printerType}打印任务>>${bitMapTaskQueue.size()}张bitmap,一张1份==总共要打印${totalPage}份==是否支持page多份${isSupportPageMore}"
                )
            }
        } else {
            //多图打印,PDF
   ...
   */
    ...
    private suspend fun turnBitmapToDuanList(
        bitmap: Bitmap, page: Int, index: Int,isPDF:Boolean
        ...
        if (builder.printerType==SyzPrinter.SYZTWOINCH){
            ...
            Log.i(TAG,"二寸机器速度模式LowMerry==${lowMerry}")
        }
        ...
        Log.i(
            TAG,
            "当前设备${builder.printerType}是否支持page多份==${isSupportPage}==图片分段压缩后的合计大小${oneBitmapTotalSize}==是否小于10K==${isSupportPage}"
        )
        //每段遍历处理数据
        duanArrayList.forEachIndexed { indexDuan, duanByteArray ->
            ...
            bitMapFenBaoList.forEachIndexed { baoIndex, baoBytes ->
                ...
                if (baoIndex == 0) {
                    //第一包传宽高
                    val baoData = if (builder.printerType == SyzPrinter.SYZTWOINCH) {
    ...
    suspend fun producePDFFile(bitmapList: MutableList<Bitmap>): SyzBitmapProcessor {
        ...
        bitmapList.forEachIndexed { index, bitmap ->
            val duanListQueue = turnBitmapToDuanList(bitmap, 1, index,true).second
            if (bitmap != null && !(bitmap.isRecycled)) {
                bitmap.recycle()//回收
            }
            bitMapTaskQueue.add(duanListQueue)
...
//            releaseResources()
    ...
    private suspend fun processBitmap(
        ...
        Log.i(
            TAG, "下发第${bitmapProcessed}张图片的第一段，总共${bitmapQueue.size()}段"
        )
        duanProcessed = 0
        consumeDuansOneBitmap()
    }
    ...
    suspend fun consumeDuansOneBitmap() = withContext(Dispatchers.IO) {
        if (currentBitmapQueue.isEmpty()) {
            ...
            if (isStopSendPackgae()) {
                ...
                Log.i(
                    TAG,
                    "取消打印回收数据=已经发送的图片数==${bitmapProcessed}==已经打印的图片数==${printerIndex}"
                )
                closePrinterTask()
            } else {
    ...
    suspend fun checkPrinterStatus(): Boolean {
        ...
        if (builder.printerType == SyzPrinter.SYZTWOINCH) {
            ...
            Log.i(
                TAG,
                "二寸检查纸张类型==设备纸张类型${paperSize}==当前打印纸张类型${builder.currentPaperSize}"
            )
            if (paperSize == builder.currentPaperSize) {
                ...
                Log.e(
                    TAG,
                    "纸张类型不一致${printerState}====${paperSize}====${builder.currentPaperSize}"
                )
