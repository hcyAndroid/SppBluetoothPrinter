...
class SyzBitmapHander private constructor(var builder: Builder) {
    ...
    suspend fun produceBitmaps2(bitmapList: MutableList<Bitmap>): SyzBitmapHander {
        initPrinterParams()
    ...
    fun setBimapCallBack(callBack: BluPrintingCallBack): SyzBitmapHander {
    ...
    suspend fun consumeDuansOneBitmap() = withContext(Dispatchers.IO) {
        if (currentBitmapQueue.isEmpty()) {
            ...
            SyzBlueToothManager.getInstance().fmWriteABF4(duanData)
        }
    }

    private suspend fun checkPrinterState(): SyzPrinterState2 {
        return SyzBlueToothManager.getInstance().getPrintStatus()
    }
