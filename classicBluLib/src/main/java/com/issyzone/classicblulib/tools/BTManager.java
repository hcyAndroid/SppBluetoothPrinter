...
public class BTManager {
    ...
    public void setSyzBluCallBack(SyzBluCallBack syzBluCallBack) {
...
//                            Log.i("绑定广播",">>>设置PIN");
//                        }catch (Exception e){
//                            Log.e("绑定广播",">>>设置PIN异常"+e.getMessage());
//                        }
                        break;
                    ...
                    case BluetoothDevice.ACTION_BOND_STATE_CHANGED:
                        //10 是未绑定  11 是正在绑定 12 是已经绑定
//                        int bondState = intent.getIntExtra(BluetoothDevice.EXTRA_BOND_STATE, -1);
//                        Log.i("绑定广播",">>>"+bondState);
                                ...
                                for (Connection connection : connections) {
                                    if (device.equals(connection.getDevice())) {
                                        ...
                                        if (!connection.isConnected()) {
                                            connection.setState(bondState == BluetoothDevice.BOND_BONDED ? Connection.STATE_PAIRED : Connection.STATE_PAIRING);
                                        }
                                        break;
                                    }
                                }
    ...
    private boolean noLocationPermission(Context context) {
        ...
        if (sdkVersion >= Build.VERSION_CODES.Q) {//target sdk版本在29以上的需要精确定位权限才能搜索到蓝牙设备
            ...
            return !PermissionChecker.hasPermission(context, Manifest.permission.ACCESS_FINE_LOCATION) && !PermissionChecker.hasPermission(context, Manifest.permission.ACCESS_COARSE_LOCATION);
        }
    }
    ...
    //处理搜索回调
    private void handleDiscoveryCallback(final boolean start, @Nullable final BluetoothDevice device, final int rssi, final int errorCode, final String errorMsg) {
    ...
    private ConnectionImpl currentConnectionImpl;

    public boolean isConnect() {
        if (currentConnectionImpl == null) {
            return false;
        }
