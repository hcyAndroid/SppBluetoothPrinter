...
object AppGlobels {
    ...
    fun getScreenResolution(): Pair<Int, Int> {
        ...
        Log.i(
            TAG,
            "当前屏幕分辨率===${width}px=====${height}px===dpi==${getDpi()}===${getApplication().resources.displayMetrics.densityDpi}"
        )

        Log.i(
            TAG,
            "当前屏幕分辨率222===${calculateDpFromDpm(width)}px=====${calculateDpFromDpm(height)}px"
        )
